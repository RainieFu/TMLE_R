...
DCDR_Multiple <-function(data, exposure, outcome, covarsT, covarsO, learners, control, num_cf){
  ...
  if(length(unique(data[[outcome]])) < 2) {
    warning("Only one unique outcome value in the data. Returning NA.")
...
DCTMLE_Multiple <-function(data, exposure, outcome, covarsT, covarsO, learners, control, num_cf, sim_id){
  print(sim_id)

  #Initialize results
  runs <- tibble(r1=double(), r0=double(), rd=double(), v1=double(), v0=double(), vd=double())

  # Define base directory for saving intermediate results
  base_dir <- paste0("DCTMLE_intermediate_results/", sim_id)
  ...
  #Run on num_cf splits
  for(cf in 1:num_cf){
    # Perform DCTMLE for the current split
    result <- DCTMLE_Single(data, exposure, outcome, covarsT, covarsO, learners, control)

    # Save intermediate result
    cf_dir <- paste0(base_dir, "/", sim_id, "_", cf)

    # Create directory for current num_cf if it doesn't exist
    if (!dir.exists(cf_dir)) {
      dir.create(cf_dir, recursive = TRUE)
    }

    # Save the result as a CSV file
    write.csv(result, file = paste0(cf_dir, "/intermediate_result.csv"), row.names = FALSE)

    # Append result to runs
    runs <- bind_rows(runs, result)
  }
...
# Single Cross-fit TMLE Estimator
SCTMLE_Single <- function(data, exposure, outcome, covarsT, covarsO, learners, control){

  # Split sample into two parts
  splits <- sample(rep(1:2, diff(floor(nrow(data) * c(0, 1/2, 2/2)))))
  data <- data %>% mutate(s = splits)
  ...
  data <- data %>%
    mutate(
      pi = case_when(
        s == 1 ~ predict(dat_nested$pi_fit[[2]], newdata = data[, covarsT])$pred,
        s == 2 ~ predict(dat_nested$pi_fit[[1]], newdata = data[, covarsT])$pred
      ),
      mu = case_when(
        s == 1 ~ predict(dat_nested$mu_fit[[2]], newdata = data[, c(exposure, covarsO)])$pred,
        s == 2 ~ predict(dat_nested$mu_fit[[1]], newdata = data[, c(exposure, covarsO)])$pred
      )
    )

  # Create datasets with statin set to 0 and 1
  dat1 <- data %>% mutate(statin = 1)
  dat0 <- data %>% mutate(statin = 0)

  # Calculate mu1 and mu0
  data <- data %>%
    mutate(
      mu1 = case_when(
        s == 1 ~ predict(dat_nested$mu_fit[[2]], newdata = dat1[, c(exposure, covarsO)])$pred,
        s == 2 ~ predict(dat_nested$mu_fit[[1]], newdata = dat1[, c(exposure, covarsO)])$pred
      ),
      mu0 = case_when(
        s == 1 ~ predict(dat_nested$mu_fit[[2]], newdata = dat0[, c(exposure, covarsO)])$pred,
        s == 2 ~ predict(dat_nested$mu_fit[[1]], newdata = dat0[, c(exposure, covarsO)])$pred
      )
    )
  ...
  data <- data %>%
    mutate(
      if1 = statin / pi * (Y - mu1_1) + mu1_1 - r1,
...
# Function that does SCTMLE over multiple sample splits and combines results
SCTMLE_Multiple <- function(data, exposure, outcome, covarsT, covarsO, learners, control, num_cf, sim_id){
  print(sim_id)
  ...
  # Define base directory for saving intermediate results
  base_dir <- paste0("SCTMLE_intermediate_results/", sim_id)
  ...
  # Run on num_cf splits
  for(cf in 1:num_cf){
    # Perform SCTMLE for the current split
    result <- SCTMLE_Single(data, exposure, outcome, covarsT, covarsO, learners, control)
